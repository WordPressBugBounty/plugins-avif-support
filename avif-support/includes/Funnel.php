	...
	class GPLS_Funnel {
		...
		public static function render() {
			...
			printf(
				...
				esc_url( $url ),
				esc_html( $offer['cta'] ),
				esc_html__( 'No thanks', 'gpls-avif-support' )
			);
		...
		private static function eligible_offer() {
			...
			foreach ( (array) self::$config['offers'] as $key => $offer ) {
				$offer = wp_parse_args(
					$offer,
					array(
						'id'      => (string) $key,
						...
						'stat'       => '',
						'stat_label' => '',
						'cta'        => __( 'Take a look', 'gpls-avif-support' ),
					)
				);
		...
		private static function stat_markup( $offer ) {
			...
			return sprintf(
				...
				esc_html( $offer['stat'] ),
				esc_html( isset( $offer['stat_label'] ) ? $offer['stat_label'] : '' )
			);
		}
